...
**<details><summary>ChatGPT 작업 처리</summary>**

❗**문제**: ChatGPT API를 사용중 일때는 요청이 처리될 때까지 다음 요청을 받을 수 없음.<br/>
<br/>
❓**해결방안**: 요청받은 질문이 증발하는 것을 막기위해 작업을 DB에 저장해 순차적으로 API요청하여 작업처리.<br/>
<br/>
❗**문제2**: ChatGPT API계정 하나로 작업을 처리하다 보니 한번에 많은 요청이 들어오면 처리속도가 느림.
❓**해결방안**: API를 늘려 작업을 분산처리 하기로 결정.<br/>
<br/>
